...
<body>
    ...
    <center>
        ...
        // Consulta SQL para contar el número total de registros en la tabla "conduce"
        $countSql = "SELECT COUNT(*) AS total FROM conduce";
        ...
        // Consulta SQL para obtener los datos de "Choferes" y "Vehículos" desde la tabla "conduce"
        $sqlConduce = "SELECT cedula, matricula FROM conduce LIMIT $offset, $itemsPerPage";
        $resultConduce = $db->query($sqlConduce);
        ...
        if ($totalPages > 0) {
            ...
            // Table (Choferes y Vehículos)
            echo "<table class='custom-table'>"; // Add a CSS class for styling
            echo "<tr><th>Choferes</th><th>Vehículos</th></tr>";

            while ($rowConduce = $resultConduce->fetch_assoc()) {
                echo "<tr><td>" . ($rowConduce["cedula"] ?? '') . "</td><td>" . ($rowConduce["matricula"] ?? '') . "</td></tr>";
            }

            echo '<tr><td colspan="2" style="text-align: center;">';
            for ($page = 1; $page <= $totalPages; $page++) {
                echo "<a class='pagination-button' href='choferesAsignados.php?page=$page'>$page</a> ";
            }
            echo '</td></tr>';
            echo "</table>";
